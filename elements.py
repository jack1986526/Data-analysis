...
atypes = node.source.particle_properties.particle_type
atypes.type_list[0].radius = 1.35        # Cu atomic radius (atom type 1 in input file)
atypes.type_list[0].name = 'Cu'
atypes.type_list[1].radius = 1.55
atypes.type_list[1].name = 'Al'        # Zr atomic radius (atom type 2 in input file)
atypes.type_list[2].radius = 1.82
atypes.type_list[2].name = 'Zr'        # Al atomic radius (3?)
...
voro_indices_all = node.output.particle_properties['Voronoi Index'].array

print (atypes.array)

# Only choose data with Cu == 1, Al == 2 and Zr == 3.
print (numpy.size(voro_indices_all))
voro_indices = []

print (numpy.where( atypes.array == 0)[0])

voro_indices = numpy.delete(voro_indices_all, numpy.where( atypes.array != 1)[0], 0)
